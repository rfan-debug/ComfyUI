...
class InstantXControlNetFlux(Flux):
    ...
    def apply_mask(self, sample, mask_pixels):
        def create_latent_mask(pixel_mask):
            ...
            downsampled_mask = F.interpolate(pixel_mask.unsqueeze(0).float(), 
                    size=target_size, 
           ...
           # print(f"After combining blocks shape: {packed_mask.shape}")
            ...
            latent_mask = downsampled_mask.reshape(-1)
            print(f"Final latent_mask shape: {latent_mask.shape}")

            return latent_mask.to(sample.device)
        latent_mask = create_latent_mask(mask_pixels)
        latent_mask = latent_mask.unsqueeze(-1).unsqueeze(0)
